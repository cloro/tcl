...
# -----
# HISTORY:
# v5.05c-corrected ID placements which fixes issues with
#        retweets no longer showing.
#       -coreccted hanging double-quote on ends of tweets.
#       -corrected other various minor things here n there.
# v5.05b-stuff.. did some stuff
# v5.05a-corrected display elements regarding
#        retweets and other details. work in
#        progess. may still need more work.
#        crosses fingers...
# v5.05- major addition: added retweets:
#	   - !RTofME - retweets of me
#	   - !RTbyME - retweets by me
#	   - !RTtoME - retweets to me
#        - !RT	 - issue a retweet
#      - major addition: direct messages:
#	   - !myprivates - show direct messages sent
#	   - !private - send a direct message
#      - minor correction: support geotagging
#      - minor correction: correct timeline ID placements
# v5.04- major addition: added search tracking:
#        - +track - add items
#        - -track - remove items
#        - !track - list items
#        - this is used to track real time search
#          results live to your channel.
# v5.03- corrected bugs: oauth package:
#        added more robust error detection within
#      - corrected errors: display is cleaner now
# v5.02- corrcected more minor bugs
#      - added two newline config options
#      - corrected mention/friend double relay
# v5.01- corrected minor bugs
#      - added new config option to oauth and twitter
#      - cleaned up displays.
# v5.0 - major addition: added oauth:
#        - provides oauth package support when sourced
#        - single token use.
#        - original oauth.tcl by horgh, modified
#          to work on eggdrop1.6.15/tcl8.4 and
#          upwards.
#        - requires only base64.tcl and sha1.tcl
#          both from tcl-lib, included.
#      - minor polish: correctly inconsitant data.
#      - minor polish: Enhanced error detection:
#	   - Fail whales will be reported properly.
#        - all error messages are verbatim from twitter along
#          with numeric code.
# ----
# v4.35- *unreleased*
# v4.34- minor polish: change some stuff to make it cleaner looking
#      - minor addition: added actions to simulate irc
# v4.33- minor polish: changed display formatting:
#        - now uses durations instead of timestamps
#      - minor polish: auto-shorten url's
#        - URL/Link's are now automatically shortened
#      - minor polish: enhanced transcode support:
#        - _ALL_ twitter entities will be properly decoded
# v4.32- major addition: added following:
#        - +follow / -follow
#        major addition: added friend/following lists:
#        - !friends / !followers
# v4.31- major addition: added other user timelines :)
#      - minor addition: corrected restAPI showing duplicate entries
# v4.3 - minor addition: corrected and clarified options:
#        - megahal @mention reply now has it's own channel
#          setting rather than config setting only.
#        minor additoon: added services list:
#        - meghal @mention reply won't do so to any usernames
#          you have listed as a service.
#          Adds: !services +service -service
#          This works like an ignore for the megahal's
#          @mention reply feature.
# v4.2 - major addition: remove private reply:
#        - now works as @mention reply, will
#          also display mentions to IRC just
#          like an rss script. Megahal can
#          be turned on or off to reply to
#          tweets from irc users within your accounts
#          or to other twitter accounts that message
#          the accounts it's tied to.
#        - Acts as a bridge between two or more different
#          IRC channels with seperate bots unlinked.
#          Merely use this script to tie your twitter
#          accounts to an irc channel then simply
#          follow any channels you wish to converse with.
#          To do this you must @mention their account
#          they will see this on irc in their channel.
#          They can then reply back to you with your
#          accounts @mention.
# v4.1 - major addition: added private reply:
#        - works via megahal, replies to those
#          who private message your bot. also
#          sends these replies, messages-id's,
#          and timestamps to correpsonding
#          channels.
# v4.0 - major addition: added megahal:
#        - replies to <nick> or normal tweets to
#          the account
#        - will not reply to it's own tweets
# ----
# v4.x: This version branch has new terms and agreements along with
#       a small disclaimer. Adding MegaHAL AI requires this to alert
#       users to the potential outcome if they act irresponsible.
# ----
# v3.3 - major addition: added timeline parsing engine
#        allowing additional functions:
#        !user !home !friends !public
#        major addition: added @reply ability using
#        msgid's to !tweet
# v3.2 - minor modification: changed the way twitter
#        syntax is expanded so nicknames tweeting are
#        no longer wrongly associated to accounts.
# v3.1 - minor addition: added localized tweetformat
#        and a corresponding global. allows variety.
# v3.0 - major addition: added tweets new name
#        officially now (twitter & tweets :P)
#        minor addition: added redirect support
#        and some encoding support for twittering.
# ----
# v2.0 - major rewrite, cleanup, bug fixes, some enhancements.
# ----
# v1.1 - minor modification html scrubber
# v1.0 - by warlord
#
# -----
# TO DO (What to expect in future updates?):
# 1 ) Add the following restAPI Features:
#     + statuses/destroy - delete tweets (irc has no delete, this is skipped for now)
#     + direct_messages/destroy - delete a private message (see above note about delete)
#     + blocks/create - block a user (ignore is useless on irc, block is like ignore, skipped for now)
#     + blocks/destroy - unlock a user (see above note about ignore)
#     + blocks/blocking - list of blocked users (see above note about ignore)
#
# 2 ) Add the following searchAPI features:
#     + trends - what's populur overall? (support pending)
#     + trends/current - what's popular this hour? (support pending)
#     + trends/daily - what's popular today? (support pending)
#     + trends/weekly - what's popular this week? (support pending)
#
# 3 ) PROFIT? (almost here, at which point.. where is the eggdrop-app-store :D.. keke)
#
# --> config begins
...
set twitter(userxml) "http://www.twitter.com/statuses/user_timeline.xml"
...
set twitter(mentionxml) "http://www.twitter.com/statuses/mentions.xml"
...
# (list)
set twitter(accts) {
	"#channel|Login|Consumer_key|Consumer_secret|Access_token|Access_token_secret[|tweet-format]" //CAMBIAR ESTO!!!
}
...
proc proc:twitter {nick uhand handle chan input} {
	if {[channel get $chan twitter]} {
		...
		if {![llength [split $input]]} {
			...
			regsub -all "\n" $html " " html
			# scrape output
			set regionalized [regexp -- {<html xmlns="http://www.w3.org/1999/xhtml".*? lang="(.*?)"} $html "" reglang]
			if {[regexp -nocase -- {<span class="label">Name.*?<span class=.*?>(.*?)</span>} $html - name]} {
				set nametag "Name"
			...
			if {![info exists location]} {
				if {[regexp -nocase -- {<span class="label">Location.*?<span class=.*?>(.*?)</span>} $html - location]} {
					set location [proc:twitter:descdecode [proc:twitter:descdecode [string trim $location]]]
					set locationtag "location"
				}
			}
			if {[regexp -nocase -- {<span id="following_count" class="stats_count numeric">(.*?) </span>} $html - following]} {set following [string trim $following]}
			if {[regexp -nocase -- {<span id="follower_count" class="stats_count numeric">(.*?) </span>} $html - followers]} {set followers [string trim $followers]}
			if {[regexp -nocase -- {<span id="lists_count" class="stats_count numeric">(.*?) </span>} $html - listed]} {set listed [string trim $listed]}
			if {[regexp -nocase -- {<span id="update_count" class="stat_count">(.*?)</span>} $html - totaltweets]} {set totaltweets [string trim $totaltweets]}
			if {[regexp -nocase -- {id="status_(.*?)"} $html - id]} {set id [string trim $id]}
			if {[regexp -nocase -- {<span class="published timestamp".*?>(.*?)</span>.*?<span>(.*?)</span>(.*?)</span>} $html - tweettime tweetplace tweetreply]} {
			...
			if {[regexp -nocase -- {<span class="bio">(.*?)</span>} $html - bio]} { set bio [string trim $bio] }
			if {[regexp -nocase -- {<span class="entry-content">(.*?)</span>} $html - ltweet]} {
			...
			if {[info exists following]} {
				...
				if {[regexp -- {<div class="protected-box">} $html]} {
					# yes
					putserv "$to :$twitter(logo) ${extra}Following: [proc:twitter:effect $following]; Followers: [proc:twitter:effect $followers]; Listed: [proc:twitter:effect $listed]; Total Tweets: [proc:twitter:effect "Tweets Protected"]."
					if {[info exists bio]} {
					...
					# no
					putserv "$to :$twitter(logo) ${extra}Following: [proc:twitter:effect $following]; Followers: [proc:twitter:effect $followers]; Listed: [proc:twitter:effect $listed]; Total Tweets: [proc:twitter:effect $totaltweets]."
					if {[info exists bio]} {
						putserv "$to :$twitter(logo) [proc:twitter:descdecode [proc:twitter:descdecode $bio]]"
					}
					# a last tweet?
				...
				if {$regionalized > 0} { append orig " \[Region: $reglang\] " }
				if {$r != 0} { append orig " \[$r redirect[expr {$r!= 1?"s":""}]\]" }
				if {[string length $twitter(hdr)]} { append orig " \[Gzip\]" }
				putserv "$to :$twitter(logo) $orig"
				# locked
...
putlog "Twitter, Tweets with Megahal v5.05a (Idea and original script by Warlord, Super-Action rocket missles by speechles) Loaded.."
